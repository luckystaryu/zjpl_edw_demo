...
object o_stg_xunjia_h {
  def main(args: Array[String]): Unit = {
    ...
    sql(
      s"""
         |Create table if not exists ods_db.o_stg_xunjia_h(
    ...
    sql("alter table ods_db.o_stg_xunjia_h drop if exists partition (etl_dt='"+yest_dt+"')")
    sql(
      s"""
         |insert into ods_db.o_stg_xunjia_h partition (etl_dt='$yest_dt')
         |select webId
         |  ,type
         |  ,labelName
         |  ,referer
