...
object o_stg_member {
  def main(args: Array[String]): Unit = {
    ...
    logger.info(yest_dt)
    val df = spark.read.format("json").load("/user/root/maidian/stg_member_"+yest_dt+".log")
    logger.info("file:///data/maidian/sourcefile/stg_member_"+yest_dt+".log")
    ...
    "title","domain","url","browserLang","browseAgent","browser","cookieEnabled","system","systemVersion","sessionId","ip","createTime")
      .write.mode(SaveMode.Overwrite).format("parquet").saveAsTable("ods_db.o_stg_member")
    spark.stop()
  }
}
