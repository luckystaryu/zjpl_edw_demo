...
object o_stg_mainfac_h {
  def main(args: Array[String]): Unit = {
    ...
    sql("alter table ods_db.o_stg_mainfac_h drop if exists partition (etl_dt='"+yest_dt+"')")
    sql(
      s"""
         |insert into ods_db.o_stg_mainfac_h partition (etl_dt='$yest_dt')
         |select webId
         |  ,type
         |  ,labelName
         |  ,referer
