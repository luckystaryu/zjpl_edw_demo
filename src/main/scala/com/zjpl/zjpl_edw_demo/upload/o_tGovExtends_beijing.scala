...
val logger: slf4j.Logger = LoggerFactory.getLogger(this.getClass)
  def main(args: Array[String]): Unit = {
    ...
    val o_tGovExtends_beijingDF = sql(
      s"""
         |select id
         |,pid
         |,title
         ...
         |  from ods_db.o_tGovExtends_beijing
       """.stripMargin)
    ...
    val deletesql="truncate table materialgov.tGovExtends_beijing"
    MySQLUtils.deleteMysqlTableData(spark.sqlContext,deletesql)
    val mysteel_dataDF = o_tGovExtends_beijingDF.write
      .format("jdbc")
      .option("url", url)
      .option("user", user)
      .option("password", password)
      .mode(SaveMode.Append)
      .option("dbtable","materialgov.tGovExtends_beijing")
      .save()
    spark.stop()
  }
}
