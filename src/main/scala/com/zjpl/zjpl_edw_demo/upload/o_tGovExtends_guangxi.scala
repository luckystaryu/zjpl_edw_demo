...
val logger: slf4j.Logger = LoggerFactory.getLogger(this.getClass)
  def main(args: Array[String]): Unit = {
    ...
    val o_tGovExtends_guangxiDF = sql(
      s"""
         |select id
         |,pid
         |,title
         ...
         |  from ods_db.o_tGovExtends_guangxi
       """.stripMargin)
    ...
    val deletesql="truncate table materialgov.tGovExtends_guangxi"
    MySQLUtils.deleteMysqlTableData(spark.sqlContext,deletesql)
    val mysteel_dataDF = o_tGovExtends_guangxiDF.write
      .format("jdbc")
      .option("url", url)
      .option("user", user)
      .option("password", password)
      .mode(SaveMode.Append)
      .option("dbtable","materialgov.tGovExtends_guangxi")
      .save()
    spark.stop()
  }
}
