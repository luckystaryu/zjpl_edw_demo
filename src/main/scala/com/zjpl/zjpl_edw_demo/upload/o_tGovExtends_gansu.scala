  ...
  def main(args: Array[String]): Unit = {
    ...
    val o_tGovExtends_gansuDF = sql(
      s"""
         ...
         |  from ods_db.o_tGovExtends_gansu
       """.stripMargin)
    ...
    val deletesql="truncate table materialgov.tGovExtends_gansu"
    MySQLUtils.deleteMysqlTableData(spark.sqlContext,deletesql)
    val mysteel_dataDF = o_tGovExtends_gansuDF.write
      .format("jdbc")
      .option("url", url)
      .option("user", user)
      .option("password", password)
      .mode(SaveMode.Append)
      .option("dbtable","materialgov.tGovExtends_gansu")
      .save()
    spark.stop()
  }
}
