...
object o_tGovMaterialBase_xinjiang {
val logger: slf4j.Logger = LoggerFactory.getLogger(this.getClass)
  def main(args: Array[String]): Unit = {
    ...
    val o_tGovMaterialBase_xinjiangDF = sql(
      s"""
         |SELECT
         |Id
         |,cid
         ...
         |,isHandle
         |,industry
         |  from ods_db.o_tGovMaterialBase_xinjiang
    ...
    val deletesql="truncate table materialgov.tGovMaterialBase_tianjin"
    MySQLUtils.deleteMysqlTableData(spark.sqlContext,deletesql)
    val mysteel_dataDF = o_tGovMaterialBase_xinjiangDF.write
      .format("jdbc")
      .option("url", url)
      .option("user", user)
      .option("password", password)
      .mode(SaveMode.Overwrite)
      .option("dbtable","materialgov.tGovMaterialBase_xinjiang")
      .save()
    spark.stop()
  }
}
