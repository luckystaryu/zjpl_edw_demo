  ...
  def main(args: Array[String]): Unit = {
    ...
    val o_tGovExtends_hubeiDF = sql(
      s"""
         ...
         |  from ods_db.o_tGovExtends_hubei
       """.stripMargin)
    ...
    val deletesql="truncate table materialgov.tGovExtends_hubei"
    MySQLUtils.deleteMysqlTableData(spark.sqlContext,deletesql)
    val mysteel_dataDF = o_tGovExtends_hubeiDF.write
      .format("jdbc")
      .option("url", url)
      .option("user", user)
      .option("password", password)
      .mode(SaveMode.Append)
      .option("dbtable","materialgov.tGovExtends_hubei")
      .save()
    spark.stop()
  }
}
