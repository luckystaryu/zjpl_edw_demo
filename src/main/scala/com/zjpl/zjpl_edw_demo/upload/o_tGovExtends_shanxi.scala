...
val logger: slf4j.Logger = LoggerFactory.getLogger(this.getClass)
  def main(args: Array[String]): Unit = {
    ...
    val o_tGovExtends_shanxiDF = sql(
      s"""
         |select id
         |,pid
         |,title
         ...
         |  from ods_db.o_tGovExtends_shanxi
       """.stripMargin)
    ...
    val deletesql="truncate table materialgov.tGovExtends_shanxi"
    MySQLUtils.deleteMysqlTableData(spark.sqlContext,deletesql)
    val mysteel_dataDF = o_tGovExtends_shanxiDF.write
      .format("jdbc")
      .option("url", url)
      .option("user", user)
      .option("password", password)
      .mode(SaveMode.Append)
      .option("dbtable","materialgov.tGovExtends_shanxi")
      .save()
    spark.stop()
  }
}
