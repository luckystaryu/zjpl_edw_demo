...
object o_tGovMaterialBase_yunnan {
val logger: slf4j.Logger = LoggerFactory.getLogger(this.getClass)
  def main(args: Array[String]): Unit = {
    ...
    val o_tGovMaterialBase_yunnanDF = sql(
      s"""
         |SELECT
         |Id
         |,cid
         ...
         |,isHandle
         |,industry
         |  from ods_db.o_tGovMaterialBase_yunnan
    ...
    val deletesql="truncate table materialgov.tGovMaterialBase_yunnan"
    MySQLUtils.deleteMysqlTableData(spark.sqlContext,deletesql)
    val mysteel_dataDF = o_tGovMaterialBase_yunnanDF.write
      .format("jdbc")
      .option("url", url)
      .option("user", user)
      .option("password", password)
      .mode(SaveMode.Append)
      .option("dbtable","materialgov.tGovMaterialBase_yunnan")
      .save()
    spark.stop()
  }
}
