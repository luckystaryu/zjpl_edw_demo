...
val logger: slf4j.Logger = LoggerFactory.getLogger(this.getClass)
  def main(args: Array[String]): Unit = {
    ...
    val o_tGovExtends_tianjinDF = sql(
      s"""
         |select id
         |,pid
         |,title
         ...
         |  from ods_db.o_tGovExtends_tianjin
       """.stripMargin)
    ...
    val deletesql="truncate table materialgov.tGovExtends_tianjin"
    MySQLUtils.deleteMysqlTableData(spark.sqlContext,deletesql)
    val mysteel_dataDF = o_tGovExtends_tianjinDF.write
      .format("jdbc")
      .option("url", url)
      .option("user", user)
      .option("password", password)
      .mode(SaveMode.Append)
      .option("dbtable","materialgov.tGovExtends_tianjin")
      .save()
    spark.stop()
  }
}
