...
object o_tGovMaterialBase_beijing {
val logger: slf4j.Logger = LoggerFactory.getLogger(this.getClass)
  def main(args: Array[String]): Unit = {
    ...
    val o_tGovMaterialBase_beijingDF = sql(
      s"""
         |SELECT
         |Id
         |,cid
    ...
    val deletesql="truncate table materialgov.tGovMaterialBase_beijing"
    MySQLUtils.deleteMysqlTableData(spark.sqlContext,deletesql)
    val mysteel_dataDF = o_tGovMaterialBase_beijingDF.write
      .format("jdbc")
      .option("url", url)
      .option("user", user)
      .option("password", password)
      .mode(SaveMode.Append)
      .option("dbtable","materialgov.tGovMaterialBase_beijing")
      .save()
    spark.stop()
  }
}
