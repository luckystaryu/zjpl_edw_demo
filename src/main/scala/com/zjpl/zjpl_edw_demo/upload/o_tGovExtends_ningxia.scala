...
val logger: slf4j.Logger = LoggerFactory.getLogger(this.getClass)
  def main(args: Array[String]): Unit = {
    ...
    val o_tGovExtends_ningxiaDF = sql(
      s"""
         |select id
         |,pid
         |,title
    ...
    val deletesql="truncate table materialgov.tGovExtends_ningxia"
    MySQLUtils.deleteMysqlTableData(spark.sqlContext,deletesql)
    val mysteel_dataDF = o_tGovExtends_ningxiaDF.write
      .format("jdbc")
      .option("url", url)
      .option("user", user)
      .option("password", password)
      .mode(SaveMode.Append)
      .option("dbtable","materialgov.tGovExtends_ningxia")
      .save()
    spark.stop()
  }
}
