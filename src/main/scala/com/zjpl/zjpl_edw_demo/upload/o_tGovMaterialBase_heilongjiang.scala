...
object o_tGovMaterialBase_heilongjiang {
val logger: slf4j.Logger = LoggerFactory.getLogger(this.getClass)
  def main(args: Array[String]): Unit = {
    ...
    val o_tGovMaterialBase_heilongjiangDF = sql(
      s"""
         |SELECT
         |Id
         |,cid
         ...
         |,isHandle
         |,industry
         |  from ods_db.o_tGovMaterialBase_heilongjiang
    ...
    val deletesql="truncate table materialgov.tGovMaterialBase_heilongjiang"
    MySQLUtils.deleteMysqlTableData(spark.sqlContext,deletesql)
    val mysteel_dataDF = o_tGovMaterialBase_heilongjiangDF.write
      .format("jdbc")
      .option("url", url)
      .option("user", user)
      .option("password", password)
      .mode(SaveMode.Append)
      .option("dbtable","materialgov.tGovMaterialBase_heilongjiang")
      .save()
    spark.stop()
  }
}
