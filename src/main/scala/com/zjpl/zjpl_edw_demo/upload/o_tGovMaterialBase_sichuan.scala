...
object o_tGovMaterialBase_sichuan {
val logger: slf4j.Logger = LoggerFactory.getLogger(this.getClass)
  def main(args: Array[String]): Unit = {
    ...
    val o_tGovMaterialBase_sichuanDF = sql(
      s"""
         |SELECT
         |Id
         |,cid
         ...
         |,isHandle
         |,industry
         |  from ods_db.o_tGovMaterialBase_sichuan
    ...
    val deletesql="truncate table materialgov.tGovMaterialBase_sichuan"
    MySQLUtils.deleteMysqlTableData(spark.sqlContext,deletesql)
    val mysteel_dataDF = o_tGovMaterialBase_sichuanDF.write
      .format("jdbc")
      .option("url", url)
      .option("user", user)
      .option("password", password)
      .mode(SaveMode.Append)
      .option("dbtable","materialgov.tGovMaterialBase_sichuan")
      .save()
    spark.stop()
  }
}
