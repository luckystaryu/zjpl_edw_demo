  ...
  def main(args: Array[String]): Unit = {
    ...
    val o_tGovExtends_henanDF = sql(
      s"""
         ...
         |  from ods_db.o_tGovExtends_henan
       """.stripMargin)
    ...
    val deletesql="truncate table materialgov.tGovExtends_henan"
    MySQLUtils.deleteMysqlTableData(spark.sqlContext,deletesql)
    val mysteel_dataDF = o_tGovExtends_henanDF.write
      .format("jdbc")
      .option("url", url)
      .option("user", user)
      .option("password", password)
      .mode(SaveMode.Append)
      .option("dbtable","materialgov.tGovExtends_henan")
      .save()
    spark.stop()
  }
}
