  ...
  def main(args: Array[String]): Unit = {
    ...
    val o_tGovExtends_jilinDF = sql(
      s"""
         ...
         |  from ods_db.o_tGovExtends_jilin
       """.stripMargin)
    ...
    val deletesql="truncate table materialgov.tGovExtends_jilin"
    MySQLUtils.deleteMysqlTableData(spark.sqlContext,deletesql)
    val mysteel_dataDF = o_tGovExtends_jilinDF.write
      .format("jdbc")
      .option("url", url)
      .option("user", user)
      .option("password", password)
      .mode(SaveMode.Append)
      .option("dbtable","materialgov.tGovExtends_jilin")
      .save()
    spark.stop()
  }
}
