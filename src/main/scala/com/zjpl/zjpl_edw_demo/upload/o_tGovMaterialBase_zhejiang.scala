...
object o_tGovMaterialBase_zhejiang {
val logger: slf4j.Logger = LoggerFactory.getLogger(this.getClass)
  def main(args: Array[String]): Unit = {
    ...
    val o_tGovMaterialBase_zhejiangDF = sql(
      s"""
         |SELECT
         |Id
         |,cid
         ...
         |,isHandle
         |,industry
         |  from ods_db.o_tGovMaterialBase_zhejiang
    ...
    val deletesql="truncate table materialgov.tGovMaterialBase_zhejiang"
    MySQLUtils.deleteMysqlTableData(spark.sqlContext,deletesql)
    val mysteel_dataDF = o_tGovMaterialBase_zhejiangDF.write
      .format("jdbc")
      .option("url", url)
      .option("user", user)
      .option("password", password)
      .mode(SaveMode.Append)
      .option("dbtable","materialgov.tGovMaterialBase_zhejiang")
      .save()
    spark.stop()
  }
}
