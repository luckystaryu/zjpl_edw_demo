  ...
  def main(args: Array[String]): Unit = {
    ...
    val o_tGovExtends_hebeiDF = sql(
      s"""
         ...
         |  from ods_db.o_tGovExtends_hebei
       """.stripMargin)
    ...
    val deletesql="truncate table materialgov.tGovExtends_hebei"
    MySQLUtils.deleteMysqlTableData(spark.sqlContext,deletesql)
    val mysteel_dataDF = o_tGovExtends_hebeiDF.write
      .format("jdbc")
      .option("url", url)
      .option("user", user)
      .option("password", password)
      .mode(SaveMode.Append)
      .option("dbtable","materialgov.tGovExtends_hebei")
      .save()
    spark.stop()
  }
}
