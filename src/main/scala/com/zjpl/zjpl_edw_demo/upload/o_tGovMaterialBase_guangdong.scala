...
object o_tGovMaterialBase_guangdong {
val logger: slf4j.Logger = LoggerFactory.getLogger(this.getClass)
  def main(args: Array[String]): Unit = {
    ...
    val o_tGovMaterialBase_guangdongDF = sql(
      s"""
         |SELECT
         |Id
         |,cid
    ...
    val deletesql="truncate table materialgov.tGovMaterialBase_guangdong"
    MySQLUtils.deleteMysqlTableData(spark.sqlContext,deletesql)
    val mysteel_dataDF = o_tGovMaterialBase_guangdongDF.write
      .format("jdbc")
      .option("url", url)
      .option("user", user)
      .option("password", password)
      .mode(SaveMode.Append)
      .option("dbtable","materialgov.tGovMaterialBase_guangdong")
      .save()
    spark.stop()
  }
}
