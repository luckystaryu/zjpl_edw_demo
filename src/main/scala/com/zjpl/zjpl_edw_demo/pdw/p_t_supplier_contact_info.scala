...
object p_t_supplier_contact_info {
  ...
  def main(args: Array[String]): Unit = {
    ...
    sql(
      s"""
         | create table if not exists pdw_db.p_t_supplier_contact_info
         |( id                 int
         ...
         | stored as parquet
       """.stripMargin)
    sql("alter table pdw_db.p_t_supplier_contact_info drop if exists partition (etl_dt='"+yest_dt02+"')")
    sql("alter table pdw_db.p_t_supplier_contact_info drop if exists partition (etl_dt='"+yest_dt+"')")
    sql(
      s"""
         |insert into pdw_db.p_t_supplier_contact_info partition (etl_dt='$yest_dt')
         ...
         |   and t1.isDeleted !=1
         |   and t1.IsAudit =1
         |   and coalesce(t1.CONTACT,'')=''
         |union all
         |select t1.id     as supplier_id
         |      ,t1.Eid    as supplier_num
         |      ,concat_ws(',',case when coalesce(FAX,'')!=''
         |                          then concat('03',':',FAX)
         |                          else null
         |                      end) as contact_type
         |      ,cast(t1.CreateOn  as timestamp)  as create_time
