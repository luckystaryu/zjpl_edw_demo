...
object o_tgovmaterialbase_beijing_h {
  ...
  def main(args: Array[String]): Unit = {
    ...
    sql(
      s"""
         ...
         | stored as parquet
       """.stripMargin)
    sql("alter table ods_db.o_tgovmaterialbase_beijing_h drop if exists partition (etl_dt='"+yest_dt02+"')")
    sql("alter table ods_db.o_tgovmaterialbase_beijing_h drop if exists partition (etl_dt='"+yest_dt+"')")
    sql(
      s"""
         |insert into table ods_db.o_tgovmaterialbase_beijing_h partition (etl_dt='$yest_dt')
         |select  Id
         |,cid
         |,subcid
         |,nid
