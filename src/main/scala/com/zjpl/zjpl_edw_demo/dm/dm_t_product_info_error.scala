...
object dm_t_product_info_error {
  ...
  def main(args: Array[String]): Unit = {
    ...
    sql(
      s"""
         ...
         | stored as parquet
       """.stripMargin)
    sql("alter table dm_db.dm_t_check_data_valid drop if exists partition (etl_dt='"+yest_dt+"',table_name='p_t_product_info')")
    sql(
      s"""
         |insert into dm_db.dm_t_check_data_valid partition (etl_dt='$yest_dt',table_name='p_t_product_info')
         ...
         |            ,t1.id   as pk_name_value
         |            ,concat_ws(',',case when coalesce(t1.material_num,'')=''
         |                                then 'E0005'
         |                                else null
         |                           end) as error_id
         |           from dm_db.dm_t_product_info_detail_error t1
         |          where etl_dt='$yest_dt') tt
         |     LATERAL VIEW explode(split(tt.error_id,',')) tt1 as error_id
       """.stripMargin)
    spark.stop()
  }
}
